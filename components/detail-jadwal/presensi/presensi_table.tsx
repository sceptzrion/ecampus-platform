...
function StatusBadge({
  ...
  if (value === "manual") {
    if (isSelf) {
      ...
      return (
        ...
        >
          <Image src="/edit.png" alt="Presensi" width={10.5} height={10.5} />
          Ambil Presensi
        </button>
      );
    }
  ...
  if (value === "pending")
    return (
      <span className="flex items-center place-self-center gap-0.5 text-[10.5px] w-fit px-1 py-0.75 rounded-sm bg-[#FFD000] text-white font-bold">
        <Image src="/lecturer.png" alt="Dosen" width={10.5} height={10.5} />
        Menunggu Validasi
      </span>
    );
    ...
    <table className="w-full border-collapse max-w-max">
      <thead className="bg-[#F3F7F9]">
        <tr className="text-left text-sm text-[#343A40] font-bold border-b-2 border-[#DEE2E6]">
          ...
          <th className="px-4 py-3">Jumlah Kehadiran</th>
          {DATES.map((d, idx) => (
        ...
        {STUDENTS.map((s, rowIdx) => {
          ...
          return (
            <tr key={s.id} className="border-b-2 border-[#EAECEF] hover:bg-[#F8FAFB] transition-colors">
              <td className="px-4 py-4 text-sm text-[#6c757d]">{rowIdx + 1}</td>
              ...
              <td className="px-4 py-4 text-sm text-[#6c757d]">{s.hadirCount}</td>
      ...
      <tfoot>
        <tr className="bg-[#F3F7F9] border-t-2 border-[#EAECEF]">
          <td className="px-4 py-3" />
          <td className="px-4 py-3 font-bold text-[#6c757d] text-sm">Topik</td>
          <td className="px-4 py-3" />
          {DATES.map((_, idx) => (
